...
**
** otp_enc: This program connects to otp_enc_d, and asks it to perform a
** one-time pad style encryption as detailed above. By itself, otp_enc doesn’t
** do the encryption - otp_end_d does. The syntax of otp_enc is as follows:
...
** or the key file is shorter than the plaintext, then it should terminate, send
** appropriate error text to stderr, and set the exit value to 1.
**
...
** other. If this happens, otp_enc should report the rejection to stderr and
** then terminate itself. In more detail: if otp_enc cannot connect to the
** otp_enc_d server, for any reason (including that it has accidentally tried
** to connect to the otp_dec_d server), it should report this error to stderr
** with the attempted port, and set the exit value to 2. Otherwise, upon
** successfully running and terminating, otp_enc should set the exit value to 0.
**
** Again, any and all error text must be output to stderr (not into the
...
void introduction(int socketFD, char** argv) {
	...
	send_message(socketFD, OTP_ENC, strlen(OTP_ENC));															// Call the send_message() function to send the process name (otp_enc)
